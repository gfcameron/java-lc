...
class TwoStringsAreClose {
  ...
  // This is one of the faster solutions, about 8x faster
  public boolean closeStrings2(String word1, String word2) {
    ...
    for (int i = 0; i < 26; i++) {
      if ((cur1[i] == 0 && cur2[i] > 0) || (cur2[i] == 0 && cur1[i] > 0)) {
        return false;
      }
    }
    Arrays.sort(cur1);
    Arrays.sort(cur2);
    return Arrays.equals(cur1, cur2);
  }
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i < testCases.length; i++) {
      ...
      boolean result2 = tsac.closeStrings2(testCases[i][0], testCases[i][1]);
      System.out.printf("Case %d: s1=%s s2=%s is close? %b %b\n", i, testCases[i][0], testCases[i][1], result, result2);
    }
  }
}